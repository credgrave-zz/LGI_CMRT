...
ContentDetails=namedtuple("ContentDetails", "program_crid series_crid EpisodeNumber mainGenre HasPreview IsPreviewOf keyWords castList") 
...
def getWriter(endDate):
    ...
    # Added C.Redgrave 20/01/2016
    # File = 
    scheduleWriter.writerow(('PublishedStartTime', 'PublishedEndTime','PublishedDuration', 'crid', 'networkid','transponderid','serviceid','fileDate','InstanceMetadataId'))
...
def processProgramInformation(pi, referencedSeriesCrids, programCrid2ContentProviderMap, parsedPrograms):
    ...
    Episode = pi.find("{%s}EpisodeOf" % rootNS)
    if Episode != None:
        EpisodeNumber = Episode.attrib.get("index")
        referencedSeriesCrids.add(EpisodeNumber)
    else:
        EpisodeNumber = ""
    ...
    # Added C.Redgrave 20/01/2016
    for KeyWord in  basicDesc.findall("{%s}Keyword" %  rootNS):
    ...
    for child in basicDesc:       
        if child.tag.endswith('CreditsList'):
            ...
            for CreditsItem in child:
                # Added C.Redgrave 20/01/2016
                if "role" in CreditsItem.attrib and  CreditsItem.attrib["role"].endswith('ACTOR'):
                    #print "Found an Actor"
                    firstname = CreditsItem.findtext( "{%s}PersonName/{%s}GivenName" % (rootNS,"urn:tva:mpeg7:2008") )
                    surname = CreditsItem.findtext( "{%s}PersonName/{%s}FamilyName" % (rootNS,"urn:tva:mpeg7:2008") )
                    ...
                    castList.append(surname + ', ' + firstname)
    ...
    # Altered C.Redgrave 20/01/2016    
    # create and store ContentDetails named tuple(program_crid series_crid mainGenre)
    contentDetails = ContentDetails(crid, EpisodeOf, EpisodeNumber, mainGenre, HasPreview, IsPreviewOf, keyWords, castList)
    ...
    if basicDesc:
        ...
        for Synopsis in  basicDesc.findall("{%s}Synopsis" %  rootNS):
            if "length" in Synopsis.attrib:
                ...
                #Added by C.Redgrave 20/01/2016
                elif synLength == 'medium':
                    mediumSynopsis = Synopsis.text
                elif synLength == 'long':    
...
# VOD_TYPE
    ...
    vodProductWriter.writerow((
            ...
            #Added by C.Redgrave 20/10/2016
            mediumSynopsis.encode('utf8').replace('\n', ' ' ),
...
# Added C.Redgrave 20/01/2016
# File = NLcontent2015-01-20.tsv
programWriter.writerow(( 'program_crid' ,'series_crid', 'mainGenre', 'durationInSeconds', 'EpisodeNumber', 'HasPreview',  'IsPreviewOf', 'keyWords', 'castList'))
...
# Added C.Redgrave 20/01/2016
# Filename = NLtitles2015-01-20.tsv
titleWriter.writerow (('crid',  'titleType', 'lang', 'titleText'))
...
# Added C.Redgrave 20/01/2016
# File = NLonDemand2015-01-20.tsv
onDemandWriter.writerow (('VodBackOfficeId','productId','programCrid','contentProvider','contentProviderId','publishedStartTime','publishedEndTime','publishedDuration','NetworkId','TransponderId','ServiceId'))
...
# Added C.Redgrave 20/01/2016
# File = NLvodProduct2015-01-20.tsv
vodProductWriter.writerow(('groupCrid','title','shortSynopsis','mediumSynopsis', 'longSynopsis', 'UPCAgeRatingCS_2008','AgeCS_2010','SUBSCRIPTION_PACKAGE_ID','VodBackOfficeId','provider','vod_type'))
...
for crid in parsedPrograms:
    ...
    programWriter.writerow(( programDetails.program_crid ,programDetails.series_crid, programDetails.mainGenre, durationInSeconds, programDetails.EpisodeNumber, programDetails.HasPreview,  programDetails.IsPreviewOf, programDetails.keyWords, programDetails.castList))
