...
# 003       26/05/2016  oskar holm          1. fix issues with linear tv not appearing
###########################################################################
...
hdfs_tva_file=$tva_home'/'${country}'/TVA_'${country}'_'${process_date}'.xml.zip'

# Grab TVA File from BDA and pull into local CMRT directory
hdfs dfs -copyToLocal ${hdfs_tva_file} $local_data
...
python tvaMain.py ${local_data}/${country}/TVA_${country}_${process_date}.xml $local_data $country $process_date 
...
echo "###################################################"
echo "# Retrieve JSON File from ditto"
echo "###################################################"
...
if wget -q "http://nl-ditto.chello.com/${country}_ditto_${json_process_date}.json"; then
    ...
    unzip -o $local_data/${country}_ditto_${json_process_date}.zip -d $local_data/json/
    mv $local_data/json/home/bigdata/group/videorep/cmrt/data/${country}_ditto_${json_process_date}.json $local_data
fi
...
python dittoMain.py $ditto_file $local_data $country $process_date
...
zip $local_data/CMRT_${country}_${process_date}.zip $local_data/CMRT_*_${country}_${process_date}.csv
